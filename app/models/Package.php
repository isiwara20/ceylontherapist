 ...
 * Pure PDO Prepared Statements & Transactions
 */
...
class Package
{
     ...
     */
    public function getServiceIds(int $packageId): array
    {
     ...
     * @return array
     */
    public function getServices(int $packageId): array
    {
     ...
     * @return bool
     */
    public function update(int $id, array $data, array $serviceIds = []): bool
    {
        $this->db->beginTransaction();
        try {
            $sql = "UPDATE packages 
    ...
    {
        $sql = "UPDATE packages SET status = IF(status = 'ACTIVE', 'INACTIVE', 'ACTIVE'), updated_at = NOW() WHERE id = :id";
        $stmt = $this->db->prepare($sql);
        $stmt->bindValue(':id', $id, PDO::PARAM_INT);
        return $stmt->execute();
    }
